...
rig.init_gemma_model(max_context_length=1024+540)
#
...
def score_param_type(expected, response, numerical=True):
    ...
    if numerical:
        # Filter and normalize numerical values based on expected
        bin_expected = {k: v for k, v in normalize_values(expected).items() if is_numerical(k)}
    else:
        # Filter and normalize verbal (non-numerical) values based on expected
        bin_expected = {k: v for k, v in normalize_values(expected).items() if
                        not is_numerical(k) and k != "ruleInstanceName"}
    bin_response = {k: v for k, v in normalize_values(response).items() if k in bin_expected}
...
def score_param_type_avg(expected, response, numerical=True):
    ...
    if numerical:
        ...
        verbal_keys = [k for k, v in expected.items() if
                       not is_numerical(k) and k.lower() != "ruleInstanceName".lower()]
...
def evaluate():
    rows = []
    for i, (row_id, type_name, expected, free_text_list) in tqdm.tqdm(enumerate(eval_data_generation[::4]), total=len(eval_data_generation[::4])):
        print(i)
        if not i % 10:
...
file_path = generate_unique_filename("output_1", "test_results")
df_results.to_csv(file_path, index=False)

file_path = generate_unique_filename("output_1", "error_param_numerical_binary_score")
df_error_param_numerical_binary_score.to_csv(file_path, index=False)

file_path = generate_unique_filename("output_1", "error_param_verbal_binary_score")
df_error_param_verbal_binary_score.to_csv(file_path, index=False)

file_path = generate_unique_filename("output_1", "error_rule_name_score")
...
file_path = generate_unique_filename("output_1", "accuracy_results",'txt')
