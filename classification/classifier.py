...
class Classifier:
    ...
    def find_rule_name_in_query(self, query) -> tuple[list[Any], bool]:
        ...
        rule_names_list = API.db_api_rule_types.get_col("type_name")
        results = []
    ...
    def using_rag(self, query):
        succeed = False
        type_names_list = API.rag_api_classification.get_closest_type_name(query=query, prefix_query=prefix_query)
    ...
    def ask_model(self, query, type_names):
        schema_a = API.db_api_rule_types.get_dict_features(type_name=type_names[0][0], feature="schema")
        description_a = API.db_api_rule_types.get_dict_features(type_name=type_names[0][0], feature="description")

        schema_b = API.db_api_rule_types.get_dict_features(type_name=type_names[1][0], feature="schema")
        description_b = API.db_api_rule_types.get_dict_features(type_name=type_names[1][0], feature="description")


        type_name = get_dict(MODELS.gemma_api.predict(prompt) + '}')[0]
        try:
            if type_name["type_name"] in API.db_api_rule_types.get_all_types_names():
